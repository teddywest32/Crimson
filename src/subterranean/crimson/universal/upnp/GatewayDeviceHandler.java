...
public class GatewayDeviceHandler extends DefaultHandler {
	...
	/**
	 * The device that should be populated with data coming from the stream being parsed
	 */
	private GatewayDevice device;

	/**
	 * Creates a new instance of GatewayDeviceHandler that will populate the fields of the
	 * supplied device
	 *
	 * @param device
	 ...
	 *
	 * Caches the element as {@link #currentElement}, and keeps track of some basic state
	 * information.
	 *
	 * @param uri
	 ...
	 * @param qName
	 *            The qualified name (with prefix), or the empty string if qualified names
	 *            are not available.
	 * @param attributes
	 ...
	 * to take specific actions at the end of each element (such as finalising a tree node
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		if (localName.compareTo("service") == 0) {
			if (device.getServiceTypeCIF() != null && device.getServiceTypeCIF().compareTo("urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1") == 0)
				state = 2;
			if (device.getServiceType() != null && device.getServiceType().compareTo("urn:schemas-upnp-org:service:WANIPConnection:1") == 0)
				state = 3;
		}
	}
