...
public class ScreenCapture {
	...
	public void clearScreen() {
		...
		for (int y = 0; y < screen.getRowScreenBlocks(); y++)
			for (int x = 0; x < screen.getColumnScreenBlocks(); x++) {
				screen.addScreenBlock(new ScreenBlock(screen.getScreenBlockName(y, x)));
				screen.getChangedScreenBlocks().addElement(screen.getScreenBlockName(y, x));
			}
	}
	...
	public Rectangle getScreenRect() {
		return new Rectangle(0, 0, (int) screen.getScreenSizeWidth(), (int) screen.getScreenSizeHeight());
	}
